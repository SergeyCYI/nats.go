...
import (
	...
	"github.com/SergeyCYI/nkeys"
	"github.com/SergeyCYI/nuid"

	"github.com/SergeyCYI/nats.go/util"
)
...
func Connect(url string, l Logger, options ...Option) (*Conn, error) {
...
func (o Options) Connect(l Logger) (*Conn, error) {
	nc := &Conn{Opts: o}
...
func (nc *Conn) doReconnect(err error, forceReconnect bool) {
	...
	for i := 0; len(nc.srvPool) > 0; {
		...
		if err != nil {
			nc.logger.Debugf("conn.doReconnect - nc.createConn: %v, since: %s", err, time.Since(now).String())
			nc.err = nil
			continue
		}

		nc.logger.Debugf("conn.doReconnect - successfull create conn, since: %s", time.Since(now).String())
		...
		nc.logger.Debugf("conn.doReconnect - successfull process connect init, since: %s", time.Since(now).String())
		...
		nc.logger.Debugf("conn.doReconnect - successfull resend subscriptions, since: %s", time.Since(now).String())

		// Now send off and clear pending buffer
		nc.err = nc.flushReconnectPendingItems()
		if nc.err != nil {
			nc.logger.Debugf("conn.doReconnect - flushReconnectPendingItems: %v, since: %s", nc.err, time.Since(now).String())
		...
		nc.logger.Debugf("conn.doReconnect -  successful flush reconnect pending items, since: %s", time.Since(now).String())

		// Done with the pending buffer
		nc.bw.doneWithPending()

		nc.logger.Debugf("conn.doReconnect -  successful done with pending, since: %s", time.Since(now).String())
		...
		nc.logger.Debugf("conn.doReconnect -  successful flush, since: %s", time.Since(now).String())
